...
import { View, Text, TouchableOpacity, Dimensions, 
        ...
        SafeAreaView, TextInput, Keyboard, ScrollView } from 'react-native'
...
import uploadSolution from '../functions/uploadSolution'
...
const AddSolutionScreen = ()=>{
    ...
    const addSolution = async()=>{ 
        ...
        try {
            setIsVisible(true)
            await uploadSolution(description,pics, userId, problemId)
                setTimeout(()=>{
                    setIsVisible(false)
                    navigation.goBack()
                },1000)
    ...
    const isShown =  description.length >0 || heading.length >0 && pics.length >0
   

    return(
            <View style={{flex:1,backgroundColor, paddingHorizontal:0.05*width}}>
                <HeaderComp heading={"Add Solution"} handlePress={()=>navigation.goBack()}/>
                ...
                <ScrollView style={{width:'100%'}}>
                    <View style={{flex:1, justifyContent:'space-between',alignItems:'center', width:'100%', paddingTop:0.06*height}}>
                        ...
                        <View style={{ height:.3*width, marginTop:.05*width}}>
                            {
                                pics.length > 1 ?(
                                    <FlatList
                                    data={pics}
                    ...
                    <View style={{width:'100%', alignItems:'center'}}>
                        { isShown?(
                            <TouchableWithoutFeedback onPress={addSolution}>
                            <View style={{alignItems:'center',justifyContent:'center',height:.055*height, width:.56*width, backgroundColor:themeColor, marginTop:.035*height, marginBottom:0.06*height}}>
                                <Text style={{color:iconCol, fontSize:.025*height}}>Add solution</Text>
                            </View>

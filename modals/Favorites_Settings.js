...
const Favorite = ({isVisible, setIsVisible,setIsLoading})=>{
                        ...
                        renderItem={({item})=>(
                            <TouchableWithoutFeedback onPress={()=>handleCheck(item)}>
                               <View style={{flexDirection:'row', alignItems:'center', marginVertical:7}}>
                                {
                                    checked.length ==0 ?[] : checked.map(item=>item.name).includes(item.name) ? <MaterialCommunityIcons name='checkbox-marked-outline' size={0.065*width} color={themeColor}/>:
